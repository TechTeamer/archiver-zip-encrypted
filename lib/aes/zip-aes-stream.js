...
const { default: constants } = require(
    path.join(
        path.dirname(require.resolve('compress-commons')),
        'archivers/zip/constants.js'
    )
);
const { default: zipUtil } = require(
    path.join(
        path.dirname(require.resolve('compress-commons')),
        'archivers/zip/util.js'
    )
);
 ...
 */
class ZipAesStream extends ZipStream {
    constructor(options = {zlib: {}}) {
        super(options);
