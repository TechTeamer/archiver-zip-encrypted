...
describe('zip-crypto', () => {
    ...
    it('should pack stream zip-crypto/unpack 7z', (done) => {
        let archive = new ZipEncrypted({zlib: {level: 8}, encryptionMethod: 'zip20', password: Buffer.from('123')});
        archive.append(fs.createReadStream('./test/resources/test.txt'),{ name: 'test.txt'});
        archive.finalize();
    ...
    it('should pack string zip-crypto/unpack 7z', (done) => {
        let archive = new ZipEncrypted({zlib: {level: 8}, encryptionMethod: 'zip20', password: Buffer.from('123')});
        archive.append('test data', {name: 'test.txt'});
        archive.finalize();
